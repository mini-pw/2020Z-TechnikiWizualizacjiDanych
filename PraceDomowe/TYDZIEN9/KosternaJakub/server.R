...
function(input, output){
  ...
  output$plot <- renderPlot({
    ...
    {
      originalPlot <- readJPEG('originalPlot.jpg') # from file
      new <- rasterGrob(originalPlot, width = unit(1, "npc"), height = unit(1, "npc"))
      p <- p + annotation_custom(new, -Inf, Inf, -Inf, Inf) # loading the background
    }
