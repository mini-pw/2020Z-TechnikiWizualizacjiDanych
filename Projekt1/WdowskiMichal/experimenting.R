...
data <- read.csv(file = "Projekt1/WdowskiMichal/data.csv", head = TRUE, stringsAsFactors = FALSE)
leagues <- read.csv(file = "Projekt1/WdowskiMichal/leagues.csv", head = TRUE, stringsAsFactors = FALSE)
...
ggplot(data = data, aes(x = League, y = Volleys)) +
    geom_boxplot()

# rozklad rzcezy w zaleznosci od regionu
# azjaci najgorsi w pilke
ggplot(data = data, aes(x = Region, y = Dribbling)) +
    geom_boxplot()
